...
# [hyc-override] - Before action for sanitizing search params to ignore malformed queries

module Blacklight::Catalog
  VALID_SORT_VALUES = %w[count index].freeze
  ...
  # own controller.
  included do
    before_action :sanitize_facet_params, only: :facet
  ...
  def sanitize_facet_params
    # Sanitize pagination - only allow positive integers
    if params.dig(:facet, :page).present?
        page_val = params[:facet][:page].to_s.to_i
        params[:facet][:page] = page_val.positive? ? page_val : 1
    end
    
    # Sanitize sort - only allow known values
    if params.dig(:facet, :sort).present?
        sort_val = params[:facet][:sort].to_s.strip.downcase
        params[:facet][:sort] = VALID_SORT_VALUES.include?(sort_val) ? sort_val : 'count'
    end
  end
