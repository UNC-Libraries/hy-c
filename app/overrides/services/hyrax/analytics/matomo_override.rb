...
require 'hyrax/analytics/matomo'

module HycMatomoMonthlyEventsOverride
  ...
  def monthly_events_for_id(id, action, date = default_date_range)
    ...
    if action == 'file-set-in-work-download'
      ...
      return hyc_downloads_by_date_to_results(
        HycDownloadStat.with_work_id_and_date(id, start_date, end_date),
        start_date,
        end_date
      )
      ...
      start_date, end_date = split_date_range(date)
      return hyc_downloads_by_date_to_results(
        HycDownloadStat.with_fileset_id_and_date(id, start_date, end_date),
        start_date,
        end_date
      )
    end
  ...
  def monthly_events(action, date = default_date_range)
    ...
    if action == 'file-set-download' || action == 'file-set-in-work-download'
      start_date, end_date = split_date_range(date)
      return hyc_downloads_by_date_to_results(
        HycDownloadStat.within_date_range(start_date, end_date),
        start_date,
        end_date
      )
    end
  ...
  def top_events(action, date = default_date_range)
    if action == 'file-set-in-work-download'
      start_date, end_date = split_date_range(date)
      return hyc_downloads_by_field_to_array(
        HycDownloadStat.within_date_range(start_date, end_date),
        :work_id
      )
    elsif action == 'file-set-download'
    ...
    # Fall back to the original Hyrax Matomo implementation for all other actions.
    super
  end
...
# Prepend the override module so its methods run before the original Hyrax Matomo class methods.
# Then re-extend Hyrax::Analytics::Matomo with the patched ClassMethods module so the new methods
# are available on the module itself.
Hyrax::Analytics::Matomo::ClassMethods.prepend(HycMatomoMonthlyEventsOverride)
Hyrax::Analytics::Matomo.extend(Hyrax::Analytics::Matomo::ClassMethods)