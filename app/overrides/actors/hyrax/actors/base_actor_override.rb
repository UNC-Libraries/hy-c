...
# https://github.com/samvera/hyrax/blob/hyrax-v5.2.0/app/actors/hyrax/actors/base_actor.rb

module HycBaseActorOverride
  # [hyc-override] Overriding to allow updated content jobs to run immediately so file reference isn't lost
  # https://github.com/samvera/hyrax/blob/hyrax-v5.2.0/app/actors/hyrax/actors/base_actor.rb
  def update(env)
    # [hyc-override] Log deleted people objects
    super(env) && log_deleted_people_objects(env.attributes, env.curation_concern.id)
  end
  ...
  def save(env, use_valkyrie: false)
    ...
    super(env, use_valkyrie: use_valkyrie)
  end
...
actor = Hyrax::Actors::BaseActor
actor.prepend(HycBaseActorOverride)  unless actor.ancestors.include?(HycBaseActorOverride)
