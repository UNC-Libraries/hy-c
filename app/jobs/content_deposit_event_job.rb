...
class ContentDepositEventJob < ContentEventJob
  def action
    "User #{link_to depositor, main_app.search_catalog_path(f: { depositor_tesim: [depositor]})} has deposited #{link_to repo_object.title.first, polymorphic_path(repo_object)}"
  end
end