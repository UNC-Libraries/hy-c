...
class ContentUpdateEventJob < ContentEventJob
  def action
    "User #{link_to depositor, main_app.search_catalog_path(f: { depositor_sim: [depositor]})} has updated #{link_to repo_object.title.first, polymorphic_path(repo_object)}"
  end
end