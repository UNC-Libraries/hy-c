...
class ProxyDepositRequest < ActiveRecord::Base
  ...
  def send_request_transfer_message_as_part_of_create
    ...
    message = "Hello,\n\n"
    message += "#{user_link} wants to transfer ownership of #{transfer_link} to you. To accept this transfer request, "
    message += "go to the Carolina Digital Repository (CDR) transfer ownership page and click \"Accept\" next to the title "
    message += "of the work. You may need to sign into the CDR by clicking on the \"Login\" link in the upper right.\n\n"
    message += "If you have questions about transferring ownership of a work, please see the "
    message += "<a href=\"https://blogs.lib.unc.edu/cdr/index.php/frequently-asked-questions/\">CDR FAQ</a>"
    Hyrax::MessengerService.deliver(::User.batch_user, receiving_user, message, "CDR work transfer request")
  end

  def send_request_transfer_message_as_part_of_update
    message = "Hello,\n\n"
    message += "Your request to transfer ownership has been #{status} by #{receiving_user.name}."
    message += " Comments: #{receiver_comment}" if receiver_comment.present?
    Hyrax::MessengerService.deliver(::User.batch_user, sending_user, message, "CDR work transfer request #{status}")
  end
