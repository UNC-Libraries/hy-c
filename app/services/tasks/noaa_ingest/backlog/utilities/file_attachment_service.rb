...
class Tasks::NoaaIngest::Backlog::Utilities::FileAttachmentService < Tasks::IngestHelperUtils::BaseFileAttachmentService
  SLEEP_INTERVAL = 0.5
  ...
  def process_record(record)
    noaa_id = record.dig('ids', 'noaa_id')
    ...
    csv_row = @csv_rows.find { |row| row['noaa_id'] == noaa_id }
    ...
    begin
      ...
      if main_file.present?
        ...
        file_path = File.join(@full_text_path, noaa_id, main_file)
      ...
      supplemental_files.each do |filename|
        ...
        file_path = File.join(@full_text_path, noaa_id, filename)
        ...
        LogUtilsHelper.double_log("Attaching supplemental file #{filename} to work #{work_id}", :info, tag: 'Noaa File Attachment Service')
      ...
      Rails.logger.error("Error processing record #{noaa_id}: #{e.message}")
      Rails.logger.error(e.backtrace.join("\n"))
      log_attachment_outcome(record,
                            category: :failed,
                            message: "Noaa File Attachment Error: #{e.message}",
                            file_name: current_file_name || 'unknown')
    end
  end

  def log_attachment_outcome(record, category:, message:, file_name:)
    # Use NOAA ID to track seen records
    noaa_id = record.dig('ids', 'noaa_id')
    tracking_key = "#{noaa_id}:#{file_name}"
