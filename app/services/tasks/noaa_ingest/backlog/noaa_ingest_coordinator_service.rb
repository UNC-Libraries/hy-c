...
class Tasks::NoaaIngest::Backlog::NoaaIngestCoordinatorService
  ...
  def initialize(config)
    ...
    @config = config
    @tracker = Tasks::NoaaIngest::Backlog::Utilities::NoaaIngestTracker.build(
        config: config,
  ...
  def load_and_ingest_metadata
    if @tracker['progress']['metadata_ingest']['completed']
      LogUtilsHelper.double_log('Metadata ingest already completed according to tracker. Skipping this step.', :info, tag: 'NoaaIngestCoordinatorService')
      return
    end
    LogUtilsHelper.double_log('Starting metadata ingest step.', :info, tag: 'NoaaIngestCoordinatorService')
    md_ingest_service = Tasks::NoaaIngest::Backlog::Utilities::MetadataIngestService.new(
        config: @config,
        tracker: @tracker,
