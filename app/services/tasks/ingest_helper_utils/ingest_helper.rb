...
module Tasks::IngestHelperUtils::IngestHelper
  ...
  def attach_file_set_to_work(work:, file_path:, user:, visibility:)
    ...
    begin
      ...
      # Just pass the file directly - don't use UploadedFile
      File.open(file_path, 'rb') do |file|
        # Use Hydra::Works directly for synchronous upload
      ...
      # Characterize synchronously
      file_set.reload
      file_set.characterize if file_set.original_file.present?
      ...
      Rails.logger.error("Error attaching file_set with file_path: #{file_path}")
