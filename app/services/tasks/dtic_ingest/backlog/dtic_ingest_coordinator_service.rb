...
class Tasks::DTICIngest::Backlog::DTICIngestCoordinatorService
  ...
  def initialize(config)
    ...
    @config = config
    @tracker = Tasks::DTICIngest::Backlog::Utilities::DTICIngestTracker.build(
        config: config,
  ...
  def run
    ...
    LogUtilsHelper.double_log('DTIC ingest workflow completed successfully.', :info, tag: 'DTICIngestCoordinator')
      ...
      LogUtilsHelper.double_log("DTIC ingest workflow failed: #{e.message}", :error, tag: 'DTICIngestCoordinator')
      raise e
  end

  def load_and_ingest_metadata
    if @tracker['progress']['metadata_ingest']['completed']
      LogUtilsHelper.double_log('Metadata ingest already completed according to tracker. Skipping this step.', :info, tag: 'DTICIngestCoordinatorService')
      return
    end
    LogUtilsHelper.double_log('Starting metadata ingest step.', :info, tag: 'DTICIngestCoordinatorService')
    md_ingest_service = Tasks::DTICIngest::Backlog::Utilities::MetadataIngestService.new(
      config: @config,
      tracker: @tracker,
  ...
  def attach_files
    if @tracker['progress']['attach_files_to_works']['completed']
      LogUtilsHelper.double_log('File attachment already completed according to tracker. Skipping this step.', :info, tag: 'DTICIngestCoordinatorService')
      return
    end
    LogUtilsHelper.double_log('Starting file attachment step.', :info, tag: 'DTICIngestCoordinatorService')
    file_attachment_service = Tasks::DTICIngest::Backlog::Utilities::FileAttachmentService.new(
      config: @config,
      tracker: @tracker,
  ...
  def format_results_and_notify
    if @tracker['progress']['send_summary_email']['completed']
      LogUtilsHelper.double_log('Result formatting and notification already completed according to tracker. Skipping this step.', :info, tag: 'DTICIngestCoordinatorService')
      return
    end
    LogUtilsHelper.double_log('Starting result formatting and notification step.', :info, tag: 'DTICIngestCoordinatorService')
    notification_service = Tasks::DTICIngest::Backlog::Utilities::NotificationService.new(
      config: @config,
      tracker: @tracker,
