...
class Tasks::OstiIngest::Backlog::OstiIngestCoordinatorService
  ...
  def initialize(config)
    ...
    @config = config
    @tracker = Tasks::OstiIngest::Backlog::Utilities::OstiIngestTracker.build(
        config: config,
  ...
  def run
    ...
    LogUtilsHelper.double_log('OSTI ingest workflow completed successfully.', :info, tag: 'OstiIngestCoordinator')
      ...
      LogUtilsHelper.double_log("OSTI ingest workflow failed: #{e.message}", :error, tag: 'OstiIngestCoordinator')
      raise e
  end

  def load_and_ingest_metadata
    if @tracker['progress']['metadata_ingest']['completed']
      LogUtilsHelper.double_log('Metadata ingest already completed according to tracker. Skipping this step.', :info, tag: 'OstiIngestCoordinatorService')
      return
    end
    LogUtilsHelper.double_log('Starting metadata ingest step.', :info, tag: 'OstiIngestCoordinatorService')
    md_ingest_service = Tasks::OstiIngest::Backlog::Utilities::MetadataIngestService.new(
        config: @config,
        tracker: @tracker,
    ...
    @tracker.save
    LogUtilsHelper.double_log('Metadata ingest step completed.', :info, tag: 'OstiIngestCoordinatorService')
  end

  def attach_files
    if @tracker['progress']['attach_files_to_works']['completed']
      LogUtilsHelper.double_log('File attachment already completed according to tracker. Skipping this step.', :info, tag: 'OstiIngestCoordinatorService')
      return
    end

    LogUtilsHelper.double_log('Starting file attachment step.', :info, tag: 'OstiIngestCoordinatorService')
    file_attachment_service = Tasks::OstiIngest::Backlog::Utilities::FileAttachmentService.new(
        config: @config,
        tracker: @tracker,
    ...
    @tracker.save
    LogUtilsHelper.double_log('File attachment step completed.', :info, tag: 'OstiIngestCoordinatorService')
  end

  def format_results_and_notify
    if @tracker['progress']['send_summary_email']['completed']
      LogUtilsHelper.double_log('Results formatting and notification already completed according to tracker. Skipping this step.', :info, tag: 'OstiIngestCoordinatorService')
      return
    end
    LogUtilsHelper.double_log('Starting results formatting and notification step.', :info, tag: 'OstiIngestCoordinatorService')
    notification_service = Tasks::OstiIngest::Backlog::Utilities::NotificationService.new(
        config: @config,
        tracker: @tracker,
    ...
    @tracker.save
    LogUtilsHelper.double_log('Results formatting and notification step completed.', :info, tag: 'OstiIngestCoordinatorService')
  end
