...
class Tasks::StacksIngest::Backlog::Utilities::NotificationService < Tasks::IngestHelperUtils::BaseIngestNotificationService
  private
  ...
  def populate_headers!(report)
    ...
    report[:headers][:total_files] = stacks_pdf_count
    report[:headers][:admin_set_title] = @tracker['admin_set_title']
  end

  def stacks_pdf_count
    # Count of Stacks CDC PDFs in the full text directory
