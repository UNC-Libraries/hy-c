...
class Tasks::StacksIngest::Backlog::StacksIngestCoordinatorService
  ...
  def initialize(config)
    ...
    @config = config
    @tracker = Tasks::StacksIngest::Backlog::Utilities::StacksIngestTracker.build(
        config: config,
  ...
  def run
    ...
    LogUtilsHelper.double_log('Stacks CDC ingest workflow completed successfully.', :info, tag: 'StacksIngestCoordinator')
    rescue => e
      LogUtilsHelper.double_log("Stacks CDC ingest workflow failed: #{e.message}", :error, tag: 'StacksIngestCoordinator')
      raise e
  end

  def load_and_ingest_metadata
    if @tracker['progress']['metadata_ingest']['completed']
      LogUtilsHelper.double_log('Metadata ingest already completed according to tracker. Skipping this step.', :info, tag: 'StacksIngestCoordinatorService')
      return
    end
    LogUtilsHelper.double_log('Starting metadata ingest step.', :info, tag: 'StacksIngestCoordinatorService')
    md_ingest_service = Tasks::StacksIngest::Backlog::Utilities::MetadataIngestService.new(
        config: @config,
        tracker: @tracker,
  ...
  def format_results_and_notify
    ...
    LogUtilsHelper.double_log('Starting results formatting and notification step.', :info, tag: 'StacksIngestCoordinatorService')
    notification_service = Tasks::StacksIngest::Backlog::Utilities::NotificationService.new(
        config: @config,
        tracker: @tracker,
