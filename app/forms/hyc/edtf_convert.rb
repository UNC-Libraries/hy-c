module Hyc
  module EdtfConvert
    ...
    def convertToEdtf(field)
      ...
      when /^\d{4}(\s+to\s+|-)\d{4}$/ # 1990 to 1995
      ...
      when /'^(jan(uary)?|feb(ruary)?|mar(ch)?|apr(il)?|may|june|july|aug(ust)?|sept(ember)?|oct(ober)?|nov(ember)?|dec(ember)?|spring|summer|fall|winter|autumn)\s*\d{4}$/i # Spring 2000, Aug 2000, July 2000
        date_parts = split_date(field)
        normalized_string = "#{date_parts[1]}-#{@month_season[date_parts[0].downcase.to_sym]}"
      when /^(jan(uary)?|feb(ruary)?|mar(ch)?|apr(il)?|may|june|july|aug(ust)?|sept(ember)?|oct(ober)?|nov(ember)?|dec(ember)?)\s+\d{1,2}(st|nd|rd|th)?,?\s*\d{4}$/ # July 1 2000, July 1st 2000, July 1, 2000, July 1st, 2000
        date_parts = split_date(field)
        ...
        if !date_parts[1].match(/st|nd|rd|th/).nil?
          ...
          normalized_string = "#{date_parts[2]}-#{@month_season[month_of_year.to_sym]}-#{day_of_month}"
        else
          normalized_string = "#{date_parts[2]}-#{@month_season[month_of_year.to_sym]}-#{date_parts[1]}}"
        end
