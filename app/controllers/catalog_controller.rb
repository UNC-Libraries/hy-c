...
class CatalogController < ApplicationController
  ...
  configure_blacklight do |config|
    ...
    config.default_solr_params = {
      qt: 'search',
      rows: 10,
      qf: 'title_tesim description_tesim creator_label_tesim keyword_tesim all_text_timv identifier_tesim'
    }
    ...
    # We have disabled the bookmark routes, so commenting out all bookmark related config
    # config.add_results_document_tool(:bookmark, partial: 'bookmark_control', if: :render_bookmarks_control?)
    config.add_results_collection_tool(:sort_widget)
    config.add_results_collection_tool(:per_page_widget)
    config.add_results_collection_tool(:view_type_group)
    # [hyc-override] disabling all of the tools_partials since hyrax doesn't use the full record page, where these would be displayed
    ...
    config.oai = {
      ...
      document: {
        ...
        set_fields: [{ label: 'Admin Set', solr_field: 'admin_set_sim' },
                     ...
                     { label: 'Worktype', solr_field: 'has_model_ssim' }]
      }
    }
  end
