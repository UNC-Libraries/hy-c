...
describe "rake proquest:ingest", type: :task do
  ...
  it "creates a new work" do
    ...
    expect(new_dissertation['date_issued']).to eq '2011'
    ...
    expect(new_dissertation['degree']).to eq 'Doctor of Philosophy'
    expect(new_dissertation['degree_granting_institution']).to eq 'University of North Carolina at Chapel Hill Graduate School'
    expect(new_dissertation['dcmi_type']).to match_array ['http://purl.org/dc/dcmitype/Text']
    expect(new_dissertation['graduation_year']).to eq '2014'
  end
end
