...
RSpec.describe Hyrax::CitationsBehaviors::Formatters::ChicagoFormatter do
  ...
  describe '#format' do
    it 'returns a citation in apa format with a doi' do
      ...
      expect(formatter.format(presenter))
        .to eq '<span class="citation-author">Depositor, A.</span> 2019. <i class="citation-title">New Article Title.</i> NC: a publisher. doi.org/some-doi'
  ...
  describe '#format_date' do
    it 'returns a date formatted for citation' do
      expect(formatter.format_date(presenter.date_issued.first)).to eq nil
    end
  end
