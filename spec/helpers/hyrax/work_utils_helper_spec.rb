...
RSpec.describe WorkUtilsHelper, type: :module do
  ...
  it 'fetches the work data correctly' do
    allow(ActiveFedora::SolrService).to receive(:get).with("id:#{mock_records[0][0]['id']}", rows: 1).and_return('response' => { 'docs' => mock_records[0] })
    ...
    result = WorkUtilsHelper.fetch_work_data_by_id(mock_records[0][0]['id'])
    expect(result).to eq(expected_work_data)
  end

  it 'logs appropriate messages for missing values' do
      # Mock the solr response to simulate a work with missing values, if it somehow makes it past the initial nil check
    allow(ActiveFedora::SolrService).to receive(:get).with("id:#{mock_records[0][0]['id']}", rows: 1).and_return('response' => { 'docs' => [] })
    allow(Rails.logger).to receive(:warn)
    result = WorkUtilsHelper.fetch_work_data_by_id(mock_records[0][0]['id'])
    expect(Rails.logger).to have_received(:warn).with("No work found associated with work id: #{mock_records[0][0]['id']}")
    expect(Rails.logger).to have_received(:warn).with("Could not find an admin set, the work with work id: #{mock_records[0][0]['id']} has no admin set name.")
    expect(result[:work_id]).to be_nil
    expect(result[:work_type]).to be_nil
    expect(result[:title]).to be_nil
    expect(result[:admin_set_id]).to be_nil
