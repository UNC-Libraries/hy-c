...
RSpec.describe WorkUtilsHelper, type: :module do
  ...
  describe '#fetch_work_data_by_fileset_id' do
    ...
    context 'when admin set is not found' do
      it 'logs an appropriate message if the work doesnt have an admin set title' do
        ...
        expect(Rails.logger).to have_received(:warn).with("Could not find an admin set, the work with id: #{fileset_ids[1]} has no admin set name.")
    ...
    it 'matches raw DOI input against full DOI URL stored in identifier_tesim' do
      ...
      expect(result[:admin_set_title]).to eq('Sample Admin Set')
    end
  end
  ...
  describe '#fetch_admin_set_by_title_or_id' do
    before do
    ...
    context 'when fetching by ID' do
      it 'returns the admin set when found' do
        result = described_class.fetch_admin_set_by_title_or_id(admin_set_id: admin_set.id)
        expect(result).to eq(admin_set)
        expect(result.id).to eq(admin_set.id)
      end

      it 'returns nil and logs warning when admin set not found' do
        allow(Rails.logger).to receive(:warn)
        result = described_class.fetch_admin_set_by_title_or_id(admin_set_id: 'nonexistent-id')
        expect(result).to be_nil
        expect(Rails.logger).to have_received(:warn).with(/Admin set not found/)
      end
    end

    context 'when fetching by title' do
      it 'returns the admin set when found' do
        result = described_class.fetch_admin_set_by_title_or_id(admin_set_title: 'Test Admin Set')
        expect(result.id).to eq(admin_set.id)
        expect(result.title).to eq(['Test Admin Set'])
      end

      it 'returns nil when admin set not found' do
        result = described_class.fetch_admin_set_by_title_or_id(admin_set_title: 'Nonexistent Title')
        expect(result).to be_nil
      end
    end

    context 'when both title and ID are provided' do
      it 'prioritizes ID over title' do
        other_admin_set = FactoryBot.create(:admin_set, title: ['Other Admin Set'])
        
        result = described_class.fetch_admin_set_by_title_or_id(
          admin_set_id: other_admin_set.id,
          admin_set_title: admin_set.title.first
        )
        expect(result.id).to eq(other_admin_set.id)
        expect(result.title).to eq(['Other Admin Set'])
      end
    end

    context 'when neither title nor ID are provided' do
      it 'returns nil' do
        result = described_class.fetch_admin_set_by_title_or_id
        expect(result).to be_nil
      end
    ...
    context 'when blank values are provided' do
      it 'returns nil for blank title' do
        result = described_class.fetch_admin_set_by_title_or_id(admin_set_title: '')
        expect(result).to be_nil
      end

      it 'returns nil for blank ID' do
        result = described_class.fetch_admin_set_by_title_or_id(admin_set_id: '')
        expect(result).to be_nil
      end
    end
  end
