...
RSpec.describe Blacklight::FacetsHelperBehavior do
  ...
  describe '#facet_display_value' do
    it "is the facet value for an ordinary facet" do
      allow(helper).to receive(:facet_configuration_for_field)
        .with('simple_field')
        .and_return(double(query: nil, date: nil, helper_method: nil, url_method: nil))
    ...
    it "extracts the configuration label for a query facet" do
      allow(helper).to receive(:facet_configuration_for_field)
        .with('query_facet')
        .and_return(double(query: { 'query_key' => { label: 'XYZ' } }, date: nil, helper_method: nil, url_method: nil))
    ...
    it "localizes the label for date-type facets" do
      allow(helper).to receive(:facet_configuration_for_field)
        .with('date_facet')
        .and_return(double('date' => true, :query => nil, :helper_method => nil, :url_method => nil))
    ...
    it "humanizes the label for date_issued_sim facet" do
      ...
      allow(helper).to receive(:facet_configuration_for_field)
        .with('date_issued_sim')
        .and_return(double('date' => false, :query => nil, :helper_method => nil, :url_method => nil))
    ...
    it "localizes the label for date-type facets with the supplied localization options" do
      allow(helper).to receive(:facet_configuration_for_field)
        .with('date_facet')
        .and_return(double('date' => { format: :short }, :query => nil, :helper_method => nil, :url_method => nil))
