...
RSpec.describe Tasks::StacksIngest::Backlog::Utilities::FileAttachmentService do
  ...
  let(:csv_data) do
    <<~CSV
      cdc_id,stacks_url,doi,pmid,pmcid,cdr_url,has_fileset,main_file,supplemental_files,supplemental_labels
      79129,https://stacks.cdc.gov/view/cdc/79129,http://dx.doi.org/10.1353/cpr.2018.0010,29606697,PMC6542568,,,cdc_79129_DS1.pdf,cdc_79129_DS2.gif|cdc_79129_DS3.jpeg|cdc_79129_DS4.gif,label1|label2|label3
      140512,https://stacks.cdc.gov/view/cdc/140512,,,,,,cdc_140512_DS1.pdf,"",""
      134910,https://stacks.cdc.gov/view/cdc/134910,,,,,,cdc_134910_DS1.pdf,index.html,index.html
  ...
  describe '#initialize' do
    it 'loads CSV rows' do
      ...
      expect(csv_rows.first['cdc_id']).to eq('79129')
    end
  ...
  describe '#process_record' do
    ...
    context 'with main file only' do
      before do
        allow(File).to receive(:exist?)
          .with('/tmp/stacks_full_text/79129/cdc_79129_DS1.pdf')
          .and_return(true)
      end
      ...
      it 'attaches main PDF and logs success' do
        ...
        expect(service).to have_received(:attach_pdf_to_work_with_file_path!).with(
          record: record,
          file_path: '/tmp/stacks_full_text/79129/cdc_79129_DS1.pdf',
          depositor_onyen: depositor.uid
        )
        expect(service).to have_received(:log_attachment_outcome).with(
          record,
    ...
    context 'with main file and supplemental files' do
      before do
        allow(File).to receive(:exist?).with('/tmp/stacks_full_text/79129/cdc_79129_DS1.pdf').and_return(true)
        allow(File).to receive(:exist?).with('/tmp/stacks_full_text/79129/cdc_79129_DS2.gif').and_return(true)
        allow(File).to receive(:exist?).with('/tmp/stacks_full_text/79129/cdc_79129_DS3.jpeg').and_return(true)
        allow(File).to receive(:exist?).with('/tmp/stacks_full_text/79129/cdc_79129_DS4.gif').and_return(true)
      end
      ...
      it 'logs info for each supplemental file' do
        ...
        expect(LogUtilsHelper).to have_received(:double_log).with(
          "Attaching supplemental file cdc_79129_DS2.gif to work #{article.id}",
          :info,
          tag: 'Stacks File Attachment Service'
        )
      end
    end
    ...
    context 'when some supplemental files are missing' do
      before do
        allow(File).to receive(:exist?).with('/tmp/stacks_full_text/79129/cdc_79129_DS1.pdf').and_return(true)
        allow(File).to receive(:exist?).with('/tmp/stacks_full_text/79129/cdc_79129_DS2.gif').and_return(true)
        allow(File).to receive(:exist?).with('/tmp/stacks_full_text/79129/cdc_79129_DS3.jpeg').and_return(false)
        allow(File).to receive(:exist?).with('/tmp/stacks_full_text/79129/cdc_79129_DS4.gif').and_return(true)
      end
    ...
    context 'when attachment raises an error' do
      ...
      it 'catches error and logs failure' do
        ...
        expect(service).to have_received(:log_attachment_outcome).with(
          record,
          category: :failed,
          message: 'Stacks File Attachment Error: Attachment failed',
          file_name: 'cdc_79129_DS1.pdf'
        )
      end
    ...
    context 'when error occurs during supplemental file processing' do
      ...
      it 'logs the correct file name that caused the error' do
        ...
        expect(service).to have_received(:log_attachment_outcome).with(
          record,
          category: :failed,
          message: 'Stacks File Attachment Error: Supplemental attachment failed',
          file_name: 'cdc_79129_DS2.gif'
        )
      end
    end
  ...
  describe '#log_attachment_outcome' do
    ...
    it 'allows same CDC ID with different filenames' do
    ...
    it 'handles nil tracking key gracefully' do
      record_no_cdc = { 'ids' => { 'work_id' => 'work123' } }
  ...
  describe '#load_csv_rows' do
    it 'loads all CSV rows with headers' do
      ...
      expect(csv_rows.size).to eq(4)
      expect(csv_rows.first['cdc_id']).to eq('79129')
      expect(csv_rows.first['main_file']).to eq('cdc_79129_DS1.pdf')
    end
