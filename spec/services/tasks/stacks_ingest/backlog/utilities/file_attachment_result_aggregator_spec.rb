...
RSpec.describe Tasks::StacksIngest::Backlog::Utilities::FileAttachmentResultAggregator do
  ...
  describe '#aggregate_results' do
    context 'with multiple files for the same work and category' do
      let(:input_data) do
        [
          { ids: { cdc_id: '79129', work_id: '474299495' }, category: 'successfully_ingested_and_attached', message: 'Main PDF successfully attached', file_name: 'cdc_79129_DS1.pdf' },
          { ids: { cdc_id: '79129', work_id: '474299495' }, category: 'successfully_ingested_and_attached', message: 'Supplemental file successfully attached', file_name: 'cdc_79129_DS2.gif' },
          { ids: { cdc_id: '79129', work_id: '474299495' }, category: 'successfully_ingested_and_attached', message: 'Supplemental file successfully attached', file_name: 'cdc_79129_DS3.jpeg' },
          { ids: { cdc_id: '79129', work_id: '474299495' }, category: 'successfully_ingested_and_attached', message: 'Supplemental file successfully attached', file_name: 'cdc_79129_DS4.gif' }
        ]
      end
      ...
      it 'groups files with the same cdc_id, work_id, category, and message' do
        ...
        expect(JsonFileUtilsHelper).to have_received(:write_jsonl) do |results, path, options|
          ...
          expect(main_group[:ids][:cdc_id]).to eq('79129')
          expect(main_group[:ids][:work_id]).to eq('474299495')
          expect(main_group[:category]).to eq('successfully_ingested_and_attached')
          expect(main_group[:filenames]).to eq(['cdc_79129_DS1.pdf'])

          supp_group = results.find { |r| r[:message] == 'Supplemental file successfully attached' }
          expect(supp_group[:filenames]).to contain_exactly('cdc_79129_DS2.gif', 'cdc_79129_DS3.jpeg', 'cdc_79129_DS4.gif')
        end
      end
    end
    ...
    context 'with different categories for the same work' do
      let(:input_data) do
        [
          { ids: { cdc_id: '134910', work_id: 'r781wg334' }, category: 'successfully_ingested_and_attached', message: 'Main PDF successfully attached', file_name: 'cdc_134910_DS1.pdf' },
          { ids: { cdc_id: '134910', work_id: 'r781wg334' }, category: 'failed', message: 'Supplemental file not found', file_name: 'index.html' }
        ]
      end
      ...
      it 'groups files separately by category' do
        ...
        expect(JsonFileUtilsHelper).to have_received(:write_jsonl) do |results, _path, _options|
          ...
          expect(success_group[:filenames]).to eq(['cdc_134910_DS1.pdf'])
    ...
    context 'with different messages for the same category' do
      let(:input_data) do
        [
          { ids: { cdc_id: '79129', work_id: 'work1' }, category: 'failed', message: 'File not found', file_name: 'missing1.pdf' },
          { ids: { cdc_id: '79129', work_id: 'work1' }, category: 'failed', message: 'Permission denied', file_name: 'missing2.pdf' }
        ]
      end
    ...
    context 'with single file per group' do
      let(:input_data) do
        [
          { ids: { cdc_id: '79129', work_id: 'work1' }, category: 'successfully_ingested_and_attached', message: 'Main PDF successfully attached', file_name: 'file1.pdf' },
          { ids: { cdc_id: '53458', work_id: 'work2' }, category: 'successfully_ingested_and_attached', message: 'Main PDF successfully attached', file_name: 'file2.pdf' }
        ]
      end
    ...
    context 'with nil values in ids' do
      let(:input_data) do
        [
          { ids: { cdc_id: nil, work_id: 'work1' }, category: 'failed', message: 'CDC ID missing', file_name: 'file1.pdf' },
          { ids: { cdc_id: nil, work_id: 'work1' }, category: 'failed', message: 'CDC ID missing', file_name: 'file2.pdf' }
        ]
      end
      ...
      it 'groups files with nil CDC IDs together' do
        aggregator.aggregate_results

        expect(JsonFileUtilsHelper).to have_received(:write_jsonl) do |results, _path, _options|
          expect(results.size).to eq(1)
          expect(results.first[:ids][:cdc_id]).to be_nil
    ...
    context 'with complex real-world data' do
      let(:input_data) do
        [
          ...
          { ids: { cdc_id: '75969', work_id: 'ft848q83z' }, category: 'successfully_ingested_and_attached', message: 'Main PDF successfully attached', file_name: 'cdc_75969_DS1.pdf' },
          { ids: { cdc_id: '75969', work_id: 'ft848q83z' }, category: 'successfully_ingested_and_attached', message: 'Supplemental file successfully attached', file_name: 'cdc_75969_DS10.xml' },
          { ids: { cdc_id: '75969', work_id: 'ft848q83z' }, category: 'successfully_ingested_and_attached', message: 'Supplemental file successfully attached', file_name: 'cdc_75969_DS11.tiff' },
          { ids: { cdc_id: '75969', work_id: 'ft848q83z' }, category: 'successfully_ingested_and_attached', message: 'Supplemental file successfully attached', file_name: 'cdc_75969_DS12.xls' },
          # Work 2: Main file only
          { ids: { cdc_id: '140512', work_id: 'd217qp94n' }, category: 'successfully_ingested_and_attached', message: 'Main PDF successfully attached', file_name: 'cdc_140512_DS1.pdf' },
          # Work 3: Failed attachment
          { ids: { cdc_id: '999999', work_id: 'failed123' }, category: 'failed', message: 'File not found', file_name: 'missing.pdf' }
        ]
      end
      ...
      it 'correctly aggregates complex scenario' do
        ...
        expect(JsonFileUtilsHelper).to have_received(:write_jsonl) do |results, _path, _options|
          ...
          supp_group = results.find { |r| r[:ids][:cdc_id] == '75969' && r[:message] == 'Supplemental file successfully attached' }
          expect(supp_group[:filenames].size).to eq(3)
          expect(supp_group[:filenames]).to contain_exactly('cdc_75969_DS10.xml', 'cdc_75969_DS11.tiff', 'cdc_75969_DS12.xls')

          # Work 75969 main file separate
          main_group = results.find { |r| r[:ids][:cdc_id] == '75969' && r[:message] == 'Main PDF successfully attached' }
          expect(main_group[:filenames]).to eq(['cdc_75969_DS1.pdf'])

          # Work 140512 standalone
          work2 = results.find { |r| r[:ids][:cdc_id] == '140512' }
          expect(work2[:filenames]).to eq(['cdc_140512_DS1.pdf'])
    ...
    it 'preserves all key fields in output' do
      input = [
        { ids: { cdc_id: '12345', work_id: 'work123' }, category: 'test_category', message: 'test message', file_name: 'file.pdf' }
      ]
      ...
      expect(JsonFileUtilsHelper).to have_received(:write_jsonl) do |results, _path, _options|
        ...
        expect(result[:ids]).to have_key(:cdc_id)
