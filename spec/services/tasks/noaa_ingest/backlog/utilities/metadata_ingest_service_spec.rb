...
RSpec.describe Tasks::NoaaIngest::Backlog::Utilities::MetadataIngestService do
  let(:admin_set) { FactoryBot.create(:admin_set, title: ['NOAA Noaa Admin Set']) }
  ...
  let(:config) do
    {
      ...
      'input_csv_path' => '/tmp/noaa_data.csv',
      'full_text_dir' => '/tmp/noaa_full_text'
    }
  end
  let(:tracker) { Tasks::NoaaIngest::Backlog::Utilities::NoaaIngestTracker.new(config) }
  let(:md_ingest_results_path) { '/tmp/noaa_md_results.jsonl' }

  let(:csv_data) do
    <<~CSV
      noaa_id,noaa_url,doi,pmid,pmcid,cdr_url,has_fileset,main_file,supplemental_files
      79129,https://noaa.noaa.gov/view/noaa/79129,http://dx.doi.org/10.1353/cpr.2018.0010,29606697,PMC6542568,,,noaa_79129_DS1.pdf,noaa_79129_DS2.gif
      140512,https://noaa.noaa.gov/view/noaa/140512,,,,,,noaa_140512_DS1.pdf,""
      151720,https://noaa.noaa.gov/view/noaa/151720,,37714542,PMC10940227,,,,"",""
      999999,https://noaa.noaa.gov/view/noaa/999999,http://dx.doi.org/10.9999/test,,,,noaa_999999_DS1.pdf,""
    CSV
  end

  let(:existing_results) do
    [
      { ids: { noaa_id: '79129', work_id: 'existing123' }, category: 'successfully_ingested_metadata_only' }.to_json
    ]
  end
  ...
  describe '#initialize' do
    it 'sets instance variables from config' do
      ...
      expect(service.instance_variable_get(:@output_dir)).to eq('/tmp/noaa_output')
      expect(service.instance_variable_get(:@input_csv_path)).to eq('/tmp/noaa_data.csv')
  ...
  describe '#identifier_key_name' do
    it 'returns "noaa_id"' do
      expect(service.identifier_key_name).to eq('noaa_id')
    end
  end
  ...
  describe '#process_backlog' do
    ...
    let(:resolved_metadata) { { 'title' => 'Test Article', 'noaa_id' => '140512' } }
    ...
    context 'when work already exists' do
      ...
      it 'skips the work and records skip result' do
        ...
        expect(Rails.logger).to have_received(:info).with(
          '[MetadataIngestService] Skipping work with noaa_id 140512 — already exists.'
        )
    ...
    context 'when metadata resolution raises error' do
      ...
      it 'handles error and records failure' do
        ...
        expect(Rails.logger).to have_received(:error).with(
          '[MetadataIngestService] Error processing work with noaa_id 140512: API error'
        )
        expect(service).to have_received(:record_result).with(
          category: :failed,
          message: 'API error',
    ...
    it 'logs article creation for each successful record' do
      ...
      expect(Rails.logger).to have_received(:info).with(
        /\[MetadataIngestService\] Created new Article .+ for publication with NOAA ID/
      ).at_least(:once)
    end
  end
  ...
  describe '#new_article' do
    ...
    it 'creates new article with private visibility' do
      service.new_article(metadata: metadata, attr_builder: attr_builder, config: config, noaa_id: noaa_id)
    ...
    it 'adds NOAA Noaa ID to identifiers' do
      identifiers = []
      allow(article).to receive(:identifier).and_return(identifiers)

      service.new_article(metadata: metadata, attr_builder: attr_builder, config: config, noaa_id: noaa_id)

      expect(identifiers).to include('NOAA ID: 140512')
    end
    ...
    it 'returns the article' do
      result = service.new_article(metadata: metadata, attr_builder: attr_builder, config: config, noaa_id: noaa_id)
  ...
  describe '#resolve_attr_builder_and_metadata_for_row' do
    let(:row) { CSV::Row.new(['noaa_id', 'doi'], ['140512', 'http://dx.doi.org/10.1234/test']) }
    let(:doi_resolver) { instance_double(Tasks::IngestHelperUtils::DoiMetadataResolver) }
    let(:oai_resolver) { instance_double(Tasks::IngestHelperUtils::OaiPmhMetadataResolver) }
    ...
    context 'when DOI is blank' do
      let(:row_no_doi) { CSV::Row.new(['noaa_id', 'doi'], ['140512', '']) }
      let(:oai_resolver) { instance_double(Tasks::IngestHelperUtils::OaiPmhMetadataResolver) }
    ...
    context 'when noaa_id is blank' do
      let(:row_no_id) { CSV::Row.new(['noaa_id', 'doi'], ['', 'http://dx.doi.org/10.1234/test']) }

      it 'raises ArgumentError' do
        expect {
          service.send(:resolve_attr_builder_and_metadata_for_row, row_no_id)
        }.to raise_error(ArgumentError, 'NOAA ID cannot be blank')
      end
    end
    ...
    context 'when doi resolver raises error' do
      ...
      it 'logs error and falls back to OAI-PMH resolver' do
        ...
        expect(Rails.logger).to have_received(:warn).with(
          '[MetadataIngestService] DOI resolution failed for DOI http://dx.doi.org/10.1234/test (NOAA ID 140512): Resolution failed. Falling back to OAI-PMH.'
        )
        expect(Tasks::IngestHelperUtils::OaiPmhMetadataResolver).to have_received(:new).with(
          id: '140512',
          identifier_key_name: 'noaa_id',
  ...
  describe '#remaining_rows_from_csv' do
    it 'returns all CSV rows when seen list is empty' do
      ...
      expect(rows.map { |r| r['noaa_id'] }).to contain_exactly('79129', '140512', '151720', '999999')
    end
    ...
    it 'filters out rows in seen list' do
      ...
      expect(rows.map { |r| r['noaa_id'] }).to contain_exactly('151720', '999999')
    end
