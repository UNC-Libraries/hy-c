...
RSpec.describe Tasks::EricIngest::Backlog::Utilities::NotificationService do
  ...
  describe '#send_mail' do
    it 'sends an email using ERICReportMailer' do
      ...
      mailer_params_double = double('ERICReportMailer params', eric_report_email: mailer_double)
      allow(ERICReportMailer).to receive(:with).with(report: report, zip_path: zip_path).and_return(mailer_params_double)

      service.send(:send_mail, report, zip_path)

      expect(ERICReportMailer).to have_received(:with).with(report: report, zip_path: zip_path)
