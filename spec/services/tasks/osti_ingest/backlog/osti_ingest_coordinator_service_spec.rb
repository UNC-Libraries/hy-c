...
RSpec.describe Tasks::OstiIngest::Backlog::OstiIngestCoordinatorService do
  let!(:log_calls) { [] }
  let(:admin_set) { FactoryBot.build(:admin_set, title: ['OSTI Ingest Admin Set']) }
  let(:config) do
    {
      ...
      'output_dir' => '/tmp/osti_ingest_output',
  ...
  let(:md_ingest_service) { instance_double(Tasks::OstiIngest::Backlog::Utilities::MetadataIngestService) }
  let(:file_attachment_service) { instance_double(Tasks::OstiIngest::Backlog::Utilities::FileAttachmentService) }
  let(:notification_service) { instance_double(Tasks::OstiIngest::Backlog::Utilities::NotificationService) }

  subject(:coordinator) { described_class.new(config) }

  before do
    allow(Tasks::OstiIngest::Backlog::Utilities::OstiIngestTracker).to receive(:build).and_return(tracker)
    # Capture log calls
  ...
  describe '#initialize' do
    it 'builds tracker and creates output directories' do
      expect(Tasks::OstiIngest::Backlog::Utilities::OstiIngestTracker).to receive(:build)
  ...
  describe '#run' do
    before do
      allow(Tasks::OstiIngest::Backlog::Utilities::MetadataIngestService).to receive(:new).and_return(md_ingest_service)
      allow(Tasks::OstiIngest::Backlog::Utilities::FileAttachmentService).to receive(:new).and_return(file_attachment_service)
      allow(Tasks::OstiIngest::Backlog::Utilities::NotificationService).to receive(:new).and_return(notification_service)
    end
    ...
    it 'executes the full ingest workflow with email suppression' do
      ...
      expect(log_calls).to include(['OSTI ingest workflow completed successfully.', :info, { tag: 'OstiIngestCoordinator' }])
    end
    ...
    it 'logs error if workflow fails' do
      ...
      expect(log_calls).to include(['OSTI ingest workflow failed: Ingest failed', :error, { tag: 'OstiIngestCoordinator' }])
    end
  end
  ...
  describe '#load_and_ingest_metadata' do
    it 'runs metadata ingest service and marks complete' do
      ...
      expect(Tasks::OstiIngest::Backlog::Utilities::MetadataIngestService).to receive(:new).and_return(md_ingest_service)
    ...
    it 'skips if already completed' do
      ...
      expect(log_calls).to include(['Metadata ingest already completed according to tracker. Skipping this step.', :info, { tag: 'OstiIngestCoordinatorService' }])
    end
  end
  ...
  describe '#attach_files' do
    it 'runs file attachment service and marks complete' do
      ...
      expect(Tasks::OstiIngest::Backlog::Utilities::FileAttachmentService).to receive(:new).and_return(file_attachment_service)
    ...
    it 'skips if already completed' do
      ...
      expect(log_calls).to include(['File attachment already completed according to tracker. Skipping this step.', :info, { tag: 'OstiIngestCoordinatorService' }])
    end
  end
  ...
  describe '#format_results_and_notify' do
    it 'runs notification service and marks complete' do
      ...
      expect(Tasks::OstiIngest::Backlog::Utilities::NotificationService).to receive(:new).and_return(notification_service)
    ...
    it 'skips if already completed' do
      ...
      expect(log_calls).to include(['Results formatting and notification already completed according to tracker. Skipping this step.', :info, { tag: 'OstiIngestCoordinatorService' }])
    end
  end
