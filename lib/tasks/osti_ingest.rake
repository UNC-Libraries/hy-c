...
desc 'Ingest new PDFs from the OSTI backlog and attach them to the Hyrax works if matched'
task osti_backlog_ingest: :environment do
  ...
  output_directory = resolve_output_directory(args, now, prefix: 'osti_backlog_ingest')
  tracker = resume_bool ? retrieve_tracker_json(output_directory) : nil
  config = build_osti_config(args, tracker, output_directory, now)

  write_intro_banner(config: config, ingest_type: 'OSTI Backlog')

  coordinator = Tasks::OstiIngest::Backlog::OstiIngestCoordinatorService.new(config)
  coordinator.run
end

def build_osti_config(args, tracker, output_dir, now)
  include Tasks::IngestHelperUtils::RakeTaskHelper
