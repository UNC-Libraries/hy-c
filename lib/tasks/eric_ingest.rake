...
task :eric_backlog_ingest, [:resume, :output_dir, :full_text_dir, :admin_set_title, :depositor_onyen] => :environment do |_task, args|
  ...
  output_directory = resolve_output_directory(args, now, prefix: 'eric_backlog_ingest')
  tracker = resume ? retrieve_tracker_json(output_directory) : nil
  config = build_eric_config(args, tracker, output_directory, now)
...
def build_eric_config(args, tracker, output_dir, now)
